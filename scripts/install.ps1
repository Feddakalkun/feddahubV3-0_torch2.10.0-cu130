...
foreach ($Node in $NodesConfig) {
    ...
    if (-not (Test-Path $NodeDir)) {
        # A local vendored copy still wins over git clone, but the repo no
        # longer ships one: those packs are other people's code under their
        # own licences, two of them with no licence at all. Cloned from source
        # instead, on the author's terms.
